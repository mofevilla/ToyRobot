...
void Controller::HandlePlace(const std::string& args) {
   ...
   try {
      ...
      // Get third token as direction if valid
      if (ToyRobot::directionMap.find(tokens.at(2)) == ToyRobot::directionMap.end()) throw std::invalid_argument("Invalid direction");
      Direction direction = ToyRobot::directionMap.at(tokens.at(2));
